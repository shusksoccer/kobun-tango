...
- **SRS-lite（間隔反復の簡易版）**: チェック日時を `koten_known_at`（キー→epoch ms）に記録。30日経過した既習意味を持つ語は「復習期限切れ」。テストの「あいまい」は `koten_weak`（番号配列）に永続化し、「覚えた」で解除。テスト設定画面の**復習キュー**（あいまい＋期限切れ）からまとめて再出題できる。日付の無い旧データは読み込み時に現在時刻を付与（そこから30日計測）。
