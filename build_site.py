...
.tag{font-size:12px;border-radius:999px;padding:4px 11px;cursor:pointer;border:1px solid var(--c,var(--rule));
...
'美・優雅・情趣':'#5f8472','よい評価':'#3f6f86','わるい評価':'#8a5a3f','心情・つらさ':'#6a6275',
'心情・いとしさ':'#9a6a5a','不安・気がかり':'#5c7066','思考・うわさ':'#4f6b86','時・季節・自然':'#6f7a5a','ことば・学問':'#7a6480'};
const FLAG_C={'現代語とギャップ大':'#8c414c','現代語と少しズレ':'#b06a3a','多義語':'#2f5670','コアで覚える':'#3f6f86',
'プラス/マイナス両義':'#8a6d3f','呼応の副詞':'#5f5480','「〜なし」型':'#5f8472','漢語サ変':'#6c655a','敬語要注意':'#8c414c'};
...
function match(w){const s=state.sel;
  ...
  if(state.q){const q=state.q.toLowerCase();
    const hay=(w.word+' '+w.meanings.join(' ')+' '+w.themes.join(' ')+' '+w.flags.join(' ')).toLowerCase();
...
function updateCount(){const n=pool().length;$('#pcount').textContent=n;$('#startBtn').disabled=n===0;}
updateCount();
...
$('#quit').onclick=()=>{$('#play').hidden=true;$('#setup').hidden=false;};
